...
export function CounselorLookup() {
  ...
  return (
    <div className="apf-section-card p-5 sm:p-6">
      ...
      <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white/75 p-4 ring-1 ring-white/80 sm:flex-row sm:items-end">
        ...
        <Button type="button" onClick={() => void go()} disabled={checking}>
          {checking ? "Checking" : "View student"}
        </Button>
      </div>
      {error ? (
        <p className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-sm font-medium text-red-700 ring-1 ring-red-100">{error}</p>
      ) : null}
    </div>
  );
}
