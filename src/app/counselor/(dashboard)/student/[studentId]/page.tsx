  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Student record</h1>
          <p className="mt-1 font-mono text-sm text-slate-600">{studentId}</p>
        </div>
        <Link
          href={`/counselor/student/${studentId}/report`}
          className="inline-flex items-center justify-center rounded-xl bg-white px-4 py-2 text-sm font-semibold text-slate-900 ring-1 ring-slate-200/90 transition hover:bg-indigo-50/80 hover:ring-indigo-200"
        >
          Printable report
        </Link>
      </div>
